...
#[test]
fn set_and_get() {
    let mut db = DB::new(&temp_db_path("set_get"), "test", DEFAULT_MAX_SEGMENT_BYTES);
...
#[test]
fn get_missing_key() {
    let db = DB::new(&temp_db_path("missing"), "test", DEFAULT_MAX_SEGMENT_BYTES);
...
fn compact_preserves_values() {
    let mut db = DB::new(&temp_db_path("preserve"), "test", DEFAULT_MAX_SEGMENT_BYTES);
    db.set("k1", "v1").unwrap();
    db.set("k2", "v2").unwrap();
...
fn compact_keeps_latest_value() {
    let mut db = DB::new(&temp_db_path("latest"), "test", DEFAULT_MAX_SEGMENT_BYTES);
    db.set("k1", "v1").unwrap();
    db.set("k1", "v2").unwrap();
...
fn compact_drops_deleted_keys() {
    let mut db = DB::new(&temp_db_path("deleted"), "test", DEFAULT_MAX_SEGMENT_BYTES);
    db.set("k1", "v1").unwrap();
    db.delete("k1").unwrap();
...
fn segment_rolls_when_full() {
    ...
    let mut db = DB::new(&path, "test", 50);
...
fn from_dir_loads_all_segments() {
    ...
    let db = DB::from_dir(&path, "test", 50).unwrap().unwrap();
    let (_, v1) = db.get("k1").unwrap().unwrap();
    let (_, v2) = db.get("k2").unwrap().unwrap();
    assert_eq!(v1, "value_one");
    assert_eq!(v2, "value_two");
