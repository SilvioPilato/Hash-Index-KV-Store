...
fn get_sstables_lists_and_sorts() {
    ...
    let _sst1 = SSTable::from_memtable(&dir, "test", &mt1).unwrap();

    // Small delay to ensure different timestamp
    std::thread::sleep(std::time::Duration::from_millis(5));

    let mt2 = make_memtable(&[("k2", "v2")]);
    let _sst2 = SSTable::from_memtable(&dir, "test", &mt2).unwrap();
...
fn get_sstables_filters_by_name() {
    ...
    SSTable::from_memtable(&dir, "alpha", &mt).unwrap();
    SSTable::from_memtable(&dir, "beta", &mt).unwrap();
